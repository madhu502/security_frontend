...
import { deleteProduct, getAllProducts, getCategoryById } from "../../apis/Api";
    ...
    const fetchCategoryNames = async () => {
      ...
      for (const product of products) {
        if (!categoryMap[product.productCategory]) {
          // Avoid re-fetching for the same category ID
          try {
            const res = await getCategoryById(product.productCategory);
            categoryMap[product.productCategory] = res.data.categoryName;
          } catch (error) {
            console.log(
              `Error fetching category name for ID ${product.productCategory}:`,
              error
            );
                ...
                <tbody>
                  {products?.map((singleProduct) => (
                    <tr key={singleProduct._id}>
                      ...
                      <td>
                        {categoryNames[singleProduct.productCategory] ||
                          "Loading..."}
                      </td>
